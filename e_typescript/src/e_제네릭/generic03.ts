...
function UpdatedUser(id: number, changes: Partial<User>) {
  const user = users[id];

  if (!user) {
    console.log('해당 id의 하용자는 없습니다.');
    return;
  }
...
UpdatedUser(1, {name: '샛초록'});
console.log(users[1]); // { name: '샛초록', age: 11 }
UpdatedUser(2, {name: '샛파랑'})
console.log(users[2]); // { name: '샛파랑', age: 22 }

UpdatedUser(1, {age: 33});
UpdatedUser(2, {age: 44});
console.log(users[1], users[2]); // { name: '샛초록', age: 33 } { name: '샛파랑', age: 44 }

UpdatedUser(1, {});
console.log(users[1]); // { name: '샛초록', age: 33 }
