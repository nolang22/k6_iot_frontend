...
console.log(arrowFunc('샛노랑'));
console.log(arrowFunc('샛초록'));
...
function select(name?: string, nickname: string = '카에루') {
  if (name) {
...
select(); // 카에루 is guest
select('샛노랑'); // 카에루 is 샛노랑 (name)
select('샛빨강'); // 카에루 is 샛빨강 (name)
...
select(undefined, '강시') // 강시 is guest
select('샛파랑', '바다') // 바다 is 샛파랑
