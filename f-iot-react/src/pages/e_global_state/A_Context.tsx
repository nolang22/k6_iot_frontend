...
//? 사용 목적
// : props drilling을 없에고 상태 소스의 단일 책임(Single Source of Truth)을 제공
...
//    : 데이터를 공유하고자 하는 밤위에 대한 Context를 생성
...
const ExampleHeader = () => {
  ...
  if (!userContext) {
    throw new Error ('UserCotext가 UserProvider 내에서 사용되지 않음');
  }

  const {user, setUser} = userContext;

  if (!user) {
    return <button onClick={() => setUser({name: '샛노랑'})}>로그인</button>
  }
...
function A_Context() {
  return (
    <div>
      ...
      {/* UserCountext가 UserProvider 내에서 사용되지 않음 */}
      {/* <ExampleHeader /> */}
    </div>
  )
}
