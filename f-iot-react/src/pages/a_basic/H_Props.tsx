...
//* 자식 컴포넌트
// : 부모로부터 사용자의 대한 데이터를 전달(props)받아 UI로 반환
// - props는 객체 형태
...
function H_Props() {
  ...
  return (
    <div>
      {/* 
        == 콘솔창 / 컴포넌트 모두 두 번 실행 ==
        : React18 이후의 StrictMode가 '개발 모드'에서 부작용 탐지를 위해 두 번 렌터링
        > 실제 배포 환경에서는 한 번만 렌더링
      */}
      <UserCard user={{ name: '샛빨강', age: 11, email: 'qwe123' }}/>
      <UserCard user={userData}/>

      {/* <Wrapper /> */}
      <Wrapper>
        <div>안녕하세요 만나서 반갑쉽다</div>
      </Wrapper>
    </div>
  )
}
