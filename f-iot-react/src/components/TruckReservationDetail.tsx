...
//* 가게(트록)의 전체 예약 정보 - 상세 페이지
// : 상세가 없으면 안내 메시지 표시
// - 상제 새로고침 버튼으로 fetchReservationByid 호출 = 단건 업데이트 가능
function TruckReservationDetail() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      console.error("예약 취소 중 오류가 발생했습니다.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className='panel'>
        ...
        <div className='detail-row'>
          <div className='detail-label'>예약</div>
        ...
        {/* 예약 취소*/}
        <button className='danger-btn' style={{ marginLeft: 8}} onClick={handleCancel} disabled={loading}>
          예약 취소
        </button>

        {/* 선택 해제 */}
        <button className='small-btn' style={{ marginLeft: 8}} onClick={() => setSelectedTimeSlot(null)}>
          선택 해제
        </button>

        {actionMessage && <div style={{marginTop: 12}} className='muted'>
            {actionMessage}
          </div>
        }
      </div>
