...
// : 대신! 기존 객체를 복사하는(prototype)해서 새로운 객체 생성 가능
...
const dog = {
  bark() {
    console.log('이것은 dog 객체입니다. 왈왈');
  }
}

const myDog = Object.create(dog);
myDog.bark();
// >> myDog는 dog를 포로토타입으로 삼은 객체 (dog가 부모 객체 역할)

//* 클래스
// : 새로운 객체를 생성하는 템플릿 역할
// > 포로토 타입 구조를 클래스라는 이름을 빌려 표현
...
let human1 = new UpperCamelCaseName('노랑색', 22, '대학생')
let human2 = new UpperCamelCaseName('보라색', 17, '고등학생')
...
let person = new Person('노랑', '샛');
console.log(person.name); // { first: '노랑', last: '샛' }