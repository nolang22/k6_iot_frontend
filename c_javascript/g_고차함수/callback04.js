...
//? cf) 비동기
// : 작업이 끝날 때까지 기다리지 않고 다음 코드를 바로 싱핼하는 방식
...
function runlater(name = '노랑색') {
  console.log(`Hello, ${name}^^`);
}

runlater(); // Hello, 노랑색^^ - 동기 프로그래밍

setTimeout(runlater, 2000); // Hello, 노랑색^^
// setTimeout(runlater('초록색'), 2000);
...
setTimeout(runlater, 3000, '초록색'); // Hello, 초록색^^

// cf) 함수 호출: 함수명(전달할 인자 나열);
// >> 호출부 () 작성 시 코드를 익는 즉시 함수 실행!
// setTimeout(runlater(), 3000, '파랑색');

function manyParam(name, age, job) {
  console.log(`${name} is ${age} years old nad is ${job}`);
}

setTimeout(manyParam, 100, '노랑색', 22, 'developer'); // 노랑색 is 22 years old nad is developer

setTimeout(() => console.log('3초 뒤 실행'),3000);
// cf) 화살표 함수에서 소괄호 생략은 매개 변수가 1개일 떄만 가능
//      >> 매개변수가 럾는 경우에는 반드시 소괄호 작성!
