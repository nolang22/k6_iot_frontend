...
const [passingCountVlaue, passingAverageValue] = result;

console.log(passingCountVlaue); // 7 
...
console.log(newNum); // [ 12, 14 ]

//* 3번 문제
// 주거진 배열을 사용하여 문제를 해결하시오.
const words = ['apple', 'monkey', 'banana', 'pig', 'grape', 'elephant'];

const filteredWords = (words, substring) => {
  // 1) 첫 번째 인자: 배열
  // 2) 두번째 인자: 필터링할 단어
  // >> words(배열)의 각 단어 중 substiong(문자열)이 포함된 단어만 필터링하여 반환

  // +) includes() 메서드 활용: 문자열. includes(문자)
  //      >> 문자열 안에 인자로 전달된 문자가 포함되어 있을 경우 true, 그렇지 않으면 flase
